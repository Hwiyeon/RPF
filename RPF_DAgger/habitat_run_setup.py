...
def run_env(envs, episode_length, observations, run_mode = 'expert', actor = None, demo=None):
    ...
    if run_mode is not 'expert':
        actor.reset()

        demo['action'] = np.array(demo['action'],dtype=np.int8)
        a = np.where(demo['action'] > 5)
        demo['action'][a] = demo['action'][a] - 5
        ref_act = np.expand_dims(np.eye(4)[demo['action']],0)
        ref_rgb = np.expand_dims(np.array(demo['rgb'],dtype=np.float32)/255.0 * 2 - 1, 0)
        actor.encode_memory([ref_rgb, ref_act])
    ...
    for t in range(episode_length):
        ...
        if run_mode is not 'expert' and actor is not None :
            actions = actor.predict_action(np.expand_dims(np.array(observations['rgb'],dtype=np.float32)/255.0*2 -1,0))
            actions = actions + 5
            #actions[np.where(actions > 0)] = actions[np.where(best_actions > 0)] + 5
            if t > max_follow_length : break
            actions[np.where(envs.get_episode_over()) == 1] = 0
        #outputs = envs.step(best_actions) if run_mode is 'expert' else envs.step(actions)
        #observations, rewards, dones, infos = [
        #    list(x) for x in zip(*outputs)
        #]
        observations, rewards, dones, infos = envs.step(best_actions) if run_mode is 'expert' else envs.step(actions[0])

        if run_mode is 'expert' and t >= 30 : break
        elif t >= max_follow_length : break

        if dones : break
        else :
            datas['rgb'].append(past_obs['rgb'])
            datas['position'].append(curr_states[0])
            datas['rotation'].append(curr_states[1])
            datas['action'].append(best_actions)
        '''
